...
func (p Notes) Breadcrumb(year, idx int) string {
	...
	return header.Items{
		...
		header.NewTextItem("Meetings Index" + postfix).Ref(true),
	}.Table(true)
}
...
func (n Note) HyperLinkReference() string {
	return n.ref()
}
...
func (n Note) Breadcrumb() string {
	...
	return header.Items{
		...
		header.NewTextItem("Meetings Index" + page),
...
func (n Note) PrevNext(notes int) header.Items {
	...
	if n.Number > 1 {
		items = append(items, header.NewTextItem("Meeting "+strconv.Itoa(n.Number-1)))
	}
...
func (n Note) HeadingMOS(page int) string {
	...
	return tex.Hypertarget(n.ref(), "") + tex.ResizeBoxW(`\myLenHeaderResizeBox`, `Meeting `+num+`\myDummyQ`)
}

func (n Note) ref() string {
	return "Meeting " + strconv.Itoa(n.Number)
}
