...
func (r Section) GenerateSection(ctx context.Context) (entities.Note, error) {
	...
	for _, day := range r.globalParameters.Months.Days() {
		headerBytes, err := r.header.GenerateComponent(ctx, day, r.parameters)
		if err != nil {
		...
		bodyBytes, err := r.body.GenerateComponent(ctx, day, r.parameters)
		if err != nil {
